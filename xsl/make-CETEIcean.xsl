...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    xmlns:eg="http://www.tei-c.org/ns/Examples" exclude-result-prefixes="t eg" version="1.0">
    ...
    <xsl:template name="elements">
        <xsl:for-each select="//*[namespace-uri(.) = 'http://www.tei-c.org/ns/1.0']">
            <xsl:if test="not(preceding::*[local-name() = local-name(current())])">"tei:<xsl:value-of select="local-name()"/>
            ...
            <xsl:call-template name="rewrite-descendent-selector">
                <xsl:with-param name="selector">
                    <xsl:call-template name="rewrite-child-selector">
                        <xsl:with-param name="selector" select="$reg"></xsl:with-param>
                    </xsl:call-template>
                </xsl:with-param>
            </xsl:call-template>
        </xsl:with-param>
